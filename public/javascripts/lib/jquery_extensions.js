	...
	$.fn.extend({
		...
		selectable: function (options) {
			...
			$(this).bind('mousedown', _.bind(function (e) {
				...
				if (options.ignore && $(e.target).closest(options.ignore).length) {
					return;
				}
				...
				if ((e.pageX > off.left + this.outerWidth() - edge) ||
					(e.pageY > off.top + this.outerHeight() - edge)) {
					return;
				}
				...
				var selectTargets = function (e) {
					if (e.pageX == ox && e.pageY == oy) {
						return;
					}

					var pos = coords(e);
					var x1 = pos.left + scrLeft,
						y1 = pos.top + scrTop,
						x2 = x1 + pos.width,
						y2 = y1 + pos.height;
					var hits = [];
					for (var i = 0; i < targets.length; i++) {
						var el = $(targets[i]);
						pos = el.offset();
						var ex1 = pos.left + scrLeft,
							ey1 = pos.top + scrTop,
							ex2 = ex1 + el.outerWidth(),
							ey2 = ey1 + el.outerHeight();

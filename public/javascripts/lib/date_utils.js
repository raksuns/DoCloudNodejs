window.DateUtils = {
	...
	create: function (format) {
		...
		format = 'return "' + format.replace(/"/g, '\\"') + '"';

		_.each(this.FORMATS, function (o) {
			format = format.replace(o[0], '"\n+ (' + o[1] + ') +\n"');
		});

		return new Function('d', format);
	},
	...
	parseRfc: function (dateString) {
		...
		if (!d) {
			throw new Error('Invalid RFC3339 Date: "' + dateString + '"');
		}
		...
		if (!d[7]) {
			return new Date(d[1], d[2] - 1, d[3], h, m, s);
		}

dc.ui.Dialog = Backbone.View.extend({
	...
	render: function (opts) {
		...
		if (!opts.noOverlay && dc.app.workspace) {
	...
	validateUrl: function (url) {
		if (dc.app.validator.check(url, 'url')) {
			return true;
		}
...
_.extend(dc.ui.Dialog, {
	...
	prompt: function (text, content, callback, options) {
		var onConfirm = callback && function (dialog) {
				return callback(dialog.val(), dialog);
			};
		return new dc.ui.Dialog(_.extend({
			mode: 'prompt',
	...
	choose: function (text, choices, callback, options) {
		return new dc.ui.Dialog(_.extend({
			...
			choices: choices,
			text: '',
			onConfirm: callback && function (dialog) {
				return callback(dialog.val());
			}
		}, options)).render();
	},

dc.ui.ShareDialog = dc.ui.Dialog.extend({
	...
	initialize: function (options) {
		...
		if (this.docs.all(function (doc) {
				return doc.get('reviewer_count') == 0;
			})) {
	...
	_observeReviewer: function (account, view) {
		...
		if (this._boundRender[account.id]) {
			account.unbind('change', this._boundRender[account.id]);
		}
	...
	_focusEmail: function (email) {
		...
		if (email)  {
			this._emailEl.val(email);
		}

		var email = this._emailEl.val();

		if (!email) {
			this.managementError(_.t('please_enter_email'));
		}
	},
	...
	_submitAddReviewer: function (callback, dismissEmpty) {
		...
		$.ajax({
			...
			success: _.bind(function (resp) {
				this._onAddSuccess(resp, callback);
			}, this),

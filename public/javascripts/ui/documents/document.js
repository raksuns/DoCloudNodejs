...
dc.ui.Document = Backbone.View.extend({
	...
	ERROR_MESSAGE: '<span class="interface">' +
		_.t('document_error_message', '<span class="text_link troubleshoot">', '</span>',
		'<span class="text_link contact_us">', '</span>') + '</span>',
	...
	render: function (options) {
		...
		if (dc.app.paginator && dc.app.paginator.mini) {
			data.title = dc.inflector.truncateWords(data.title, 50);
		}
		...
		if (options.notes && this.model.hasLoadedNotes()) {
	...
	viewChosenPages: function () {
		...
		dc.app.paginator.ensureRows(function () {
			_.each(docs, function (doc) {
				if (doc = Documents.get(doc.id)) {
					doc.trigger('view:pages');
				}
			});
		}, docs[0]);
	},
	...
	showMenu: function (e) {
		...
		items.push({
			title: _.t('view_entities'),
			onClick: this.viewEntities
		});

		items.push({
			title: _.t('view_pages'),
	...
	_iconAttributes: function () {
		...
		switch (access) {
			...
			case dc.access.ERROR:
				return {
					'class': base + 'alert_gray',
					title: _.t('broken_document')
				};
			case dc.access.ORGANIZATION:
				return {
					'class': base + 'lock',
					title: _.t('private_to', (dc.account ? dc.account.organization().get('name') : 'your organization'))
				};
			...
			default:
				if (this.model.isPublished()) {
					return {
						'class': base + 'published',
						title: _.t('open_published')
					};
				}
				return {
					'class': base + 'hidden',
	...
	_displayDescription: function () {
		...
		if (this.model.get('access') == dc.access.ERROR) {
			return el.html(this.ERROR_MESSAGE);
		}
	...
	_onDrop: function (e) {
		...
		if (selected.length && _.include(selected, this.model)) {
			docs = selected;
		}

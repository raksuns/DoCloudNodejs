dc.ui.DocumentDataDialog = dc.ui.Dialog.extend({
	...
	constructor: function (docs) {
		...
		dc.ui.Dialog.call(this, {
			mode: 'custom',
			title: _.t('edit_data_for', this._title()),
			saveText: _.t('save')
		});
	...
	render: function () {
		...
		var removeAll = _.any(this.docs, function (doc) {
			return !_.isEmpty(doc.get('data'));
		});
	...
	confirm: function () {
		...
		var forbidden = _.detect(_.keys(data), function (key) {
			return _.include(dc.searchPrefixes, key.toLowerCase());
		});
	...
	_title: function () {
		if (this.multiple) {
			return _.t('x_documents', this.docs.length);
		}

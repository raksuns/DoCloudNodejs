...
dc.ui.DocumentEmbedDialog = dc.ui.Dialog.extend({
	...
	STEPS: [null, null, _.t('embed_step_two_title'), _.t('embed_step_three_title')],
	...
	render: function () {
		...
		if (dc.app.preferences.get('document_embed_options')) {
	...
	displayTitle: function () {
		if (this.currentStep == 1) {
			return _.t("embed_step_one_title", dc.inflector.truncate(this.model.get('title'), 25));
		}
	...
	embedOptions: function () {
		...
		if (openToPage) {
			options.page = parseInt(openToPage, 10);
		}
	...
	_renderEmbedCode: function () {
		...
		var serialized = _.map(options, function (value, key) {
			return key + ': ' + value;
		});

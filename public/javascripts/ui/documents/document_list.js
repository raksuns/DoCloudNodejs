...
dc.ui.DocumentList = Backbone.View.extend({
	...
	_maybeSelectOrDelete: function (e) {
		...
		if (!(cmdA || del) || $(e.target).closest('input, textarea').length) {
			return;
		}
	...
	_endDeselect: function (e) {
		...
		if ($(e.target).hasClass('doc_title') || $(e.target).hasClass('doc') || $(e.target).hasClass('edit_glyph')) {
			return;
		}

		if ((Math.abs(e.pageX - this._pageX) > this.SLOP) || (Math.abs(e.pageY - this._pageY) > this.SLOP)) {
			return;
		}

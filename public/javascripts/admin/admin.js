dc.ui.Admin = Backbone.View.extend({
	...
	ACCOUNT_COMPARATORS: {
		...
		email: function (account) {
			return account.get('email').toLowerCase();
		},
		organization: function (account) {
			return account.get('orgnization_name').toLowerCase();
		},
		document_count: function (account) {
			return -(account.get('public_document_count') || 0 + account.get('private_document_count') || 0);
		},
		page_count: function (account) {
			return -account.get('page_count') || 0;
		}
	},
	...
	_createActionsMenu: function () {
		return new dc.ui.Menu({
			...
			items: [
				{
					title: 'Add an Organization', onClick: function () {
					window.location = '/admin/signup';
				}
				},
				{
					title: 'View CloudCrowd Console', onClick: function () {
					window.location = CLOUD_CROWD_SERVER;
				}
				},
				...
				{
					title: 'Edit Featured Reporting', onClick: function () {
					window.location = '/admin/featured';
				}
				}
			]
		});

/**
 * dc.model.Project = Backbone.Model
 * dc.model.ProjectSet = Backbone.Collection
...
dc.model.Project = Backbone.Model.extend({
	...
	set: function (attrs, options) {
		...
		if (attrs.title) {
			attrs.title = dc.inflector.trim(attrs.title).replace(/"/g, '');
		}

		if (attrs.account_id) {
			attrs.owner = attrs.account_id == dc.account.id;
		}
...
dc.model.ProjectSet = Backbone.Collection.extend({
	...
	startingWith: function (prefix) {
		...
		return this.select(function (m) {
			return !!m.get('title').match(matcher);
		});
	},

/**
 * dc.model.Organization = Backbone.Model
 * dc.model.OrganizationSet = Backbone.Collection
...
dc.model.Organization = Backbone.Model.extend({
	...
	statistics: function () {
		...
		var stats = [];
		if (accounts) {
			stats.push(_.t('x_accounts', accounts));
		}
		if (docs) {
			stats.push(_.t('x_documents', docs));
		}
		if (notes) {
			stats.push(_.t('x_notes', notes));
		}
		return stats.join(', ');
	}
...
dc.model.OrganizationSet = Backbone.Collection.extend({
	...
	findBySlug: function (slug) {
		return this.detect(function (org) {
			return org.get('slug') == slug;
		});
	}

 ...
 * dc.model.Document = Backbone.Model
 * dc.model.DocumentSet <extend dc.model.Selectable> = Backbone.Collection
 */
...
dc.model.Document = Backbone.Model.extend({
	...
	constructor: function (attrs, options) {
		...
		if (attrs.annotation_count == null) {
			attrs.annotation_count = 0;
		}
	...
	publicUrl: function (url) {
		if (url.indexOf('s3.documentcloud.org') < 0) {
			return url;
		}
	...
	openPublishedViewer: function () {
		...
		if (!this.isPublished()) {
			return dc.ui.Dialog.alert(_.t('x_is_not_published', this.get('title')));
		}
	...
	uniquePageEntityValues: function () {
		return _.uniq(this.pageEntities.map(function (m) {
			return m.get('value');
		}));
	},
...
dc.model.DocumentSet = Backbone.Collection.extend({
	...
	pending: function () {
		return this.select(function (doc) {
	...
	sharedAttribute: function (docs, attr) {
		var attrs = _.uniq(_.map(docs, function (doc) {
			return doc.get(attr);
		}));
	...
	allowedToEdit: function (docs, message) {
		return !_.any(docs, function (doc) {
			return !doc.checkAllowedToEdit(message);
		});
	},
	...
	printNotes: function () {
		...
		var params = _.map(docs, function (doc) {
			return 'docs[]=' + doc.id;
		}).join('&');
	...
	poll: function () {
		...
		$.get('/documents/status.json', {'ids[]': ids}, _.bind(function (resp) {
			_.each(resp.documents, function (json) {
				...
				if (doc && doc.get('access') != json.access) {
					doc.set(json);
				}
			});
	...
	editAccess: function (docs, callback) {
		...
		dc.ui.Dialog.choose(_.t('access_level'), [
			...
			_.each(docs, function (doc) {
				doc.save({access: parseInt(access, 10)});
			});

			var notification = _.t('document_access_updated', docs.length);

			if (!_.any(docs, function (doc) {
					return doc.suppressNotifier;
				})) {

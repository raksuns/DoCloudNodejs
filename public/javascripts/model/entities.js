/**
 * dc.model.Entity = Backbone.Model
 * dc.model.EntitySet = Backbone.Collection
...
dc.model.Entity = Backbone.Model.extend({
	...
	loadExcerpt: function (occurrence, callback) {
		var excerpt;
		if (excerpt = this.excerpts[occurrence]) {
			return callback(excerpt);
		}
	...
	_fetch: function (ids, options, callback) {
		...
		$.get('/documents/entity.json', data, function (resp) {
			callback(_.map(resp.entities, function (obj) {
				return new dc.model.Entity(obj);
			}));
			dc.ui.spinner.hide();
		}, 'json');
	}
...
dc.model.EntitySet = Backbone.Collection.extend({
	...
	index: function () {
		if (!this._index) {
			var index = this._index = _.groupBy(this.models, function (e) {
				return e.attributes.kind;
			});
	...
	populateDocuments: function (docs) {
		docs = _.compact(_.map(docs, function (doc) {
			return Documents.get(doc.id);
		}));

		var callback = function () {
			_.each(docs, function (doc) {
				doc.entities.trigger('load');
			});
		};

		var missing = _.select(docs, function (doc) {
			return !doc.entities.loaded;
		});
